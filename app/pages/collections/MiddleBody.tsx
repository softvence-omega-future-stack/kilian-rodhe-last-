...
// Images
import image1 from "@/public/image/collections/imag1.jpg";
import image2 from "@/public/image/collections/imag2.jpg";
import image3 from "@/public/image/collections/image.jpg";
import image4 from "@/public/image/collections/image4.jpg";
...
for (let i = 0; i < TOTAL_PRODUCTS; i++) {
  ...
  products.push({
    ...base,
    id: i + 1,
    subtitle: `${base.subtitle} ${Math.floor(i / baseProducts.length) + 1}`,
    isNew: i < 4 ? base.isNew : false,
    isBestSeller: i === 0 || i === 4 ? base.isBestSeller : false,
  });
}
...
const ColorSwatch: React.FC<ColorSwatchProps> = ({ hex, isWhite }) => (
  <div
...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  const isDarkImage = product.id === 2 || product.id === 6;
  ...
  return (
    <div className="flex flex-col border-none">
      <div className="relative overflow-hidden">
        <div
          className="w-full h-[400px] bg-cover bg-center transition duration-300"
          style={{
        ...
        >
          ...
          <div className="absolute top-3 right-3 flex space-x-2">
            <button className={`${iconButtonColor} p-2 shadow-sm transition`}>
              <Image src={heart} alt="icon" className="h-4 w-4" />
            </button>
            <button className={`${iconButtonColor} p-2 shadow-sm transition`}>
              <Image src={shop} alt="icon" className="h-4 w-4" />
            </button>
          </div>

          <div className="absolute bottom-0 w-full px-8 mb-4">
            <button
              className={`${cormorantNormal.className} w-full py-3 text-sm font-semibold tracking-widest flex justify-center items-center border border-white text-white bg-black/50 hover:bg-black/70 transition`}
            >
              CUSTOMIZE →
            </button>
          </div>
...
// ----------------------------------------------------------------------
// Pagination Component (Warning Fixed)
// ----------------------------------------------------------------------

const Pagination: React.FC<{ totalProducts: number; productsPerPage: number }> =
  ({ totalProducts, productsPerPage }) => {
    const displayedCount = productsPerPage;
    ...
    return (
      <div className="flex flex-col items-center py-12">
        ...
        <button
          className={`${jostFont.className} mb-4 px-12 py-3 border border-gray-900 text-gray-900 font-medium tracking-widest flex items-center hover:bg-gray-50 transition`}
        >
          LOAD MORE PRODUCTS
          <svg
...
export default function ShopPage() {
  const productsToDisplay = products.slice(0, PRODUCTS_PER_PAGE);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-gray-300/60">
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-700">
            ...
            <div className="relative">
              <select className="appearance-none bg-white py-2 pl-4 pr-8 border border-gray-300/40 focus:outline-none cursor-pointer text-sm">
          ...
          <div className="w-full sm:w-1/4 mb-6 sm:mb-0">
            ...
            <ul className="space-y-1 text-sm tracking-wide text-gray-800">
              {productTypes.map((type) => (
                <li key={type} className="hover:text-black cursor-pointer">
                  {type}
                </li>
              ))}
            </ul>
          ...
          <div className="w-full sm:w-1/4 mb-6 sm:mb-0">
            ...
            <ul className="space-y-1 text-sm tracking-wide text-gray-800">
              {priceRanges.map((range) => (
                <li key={range} className="hover:text-black cursor-pointer">
                  {range}
                </li>
              ))}
            </ul>
